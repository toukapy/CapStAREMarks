...
class OnlineCentering:
    ...
    def update(self, x):
        ...
        return x - self.mu  # devuelve x centrado
...
class SequenceTransform:
    ...
    def __call__(self, frames, landmarks=None, orig_hw=None):
        ...
        flip = False  # 🔴 DESACTIVAMOS flip horizontal de momento
...
if __name__ == "__main__":
    ...
    # Transformaciones
    seq_transform = SequenceTransform(size=(224,224))
    ...
    train_loader = DataLoader(
        ...
        shuffle=True,
        drop_last=True,
        num_workers=4,  # ↓ baja de 8 → 4
        pin_memory=True,
        persistent_workers=False,  # ⚠️ desactiva persistencia
    ...
    val_loader = DataLoader(
        val_dataset,
        batch_size=8,  # ↓ más pequeño que train
        shuffle=False,
        drop_last=False,
        num_workers=2,  # ↓ baja a 2
        pin_memory=True,
        persistent_workers=False,  # ⚠️ evita fugas
        prefetch_factor=1,  # solo 1 batch adelantado
        worker_init_fn=dataloader_worker_init,
        multiprocessing_context=mp_ctx,
    )
    # Modelo
    ...
    model = GazeEstimationModel(encoder, output_dim=3, num_capsules=6)  # o el N que uses

    model = model.to(device)

    # ---- calcular flattened_dim ----
    with torch.no_grad():
        dummy = torch.randn(1, 3, 224, 224, device=device)  # input y módulo en el MISMO device
        model.encoder.eval()
        feat = model.encoder(dummy)  # <- ya no hay mezcla CPU/CUDA
        flattened_dim = feat.reshape(feat.size(0), -1).size(1)
        model.encoder.train()

    model.set_capsule_input_dim(flattened_dim)

    checkpoint = torch.load('09112025.pth')
    ...
    optimizer = torch.optim.AdamW([
        ...
        # NUEVO:
        {"params": model.lmk_fuse.parameters(), "lr": 1e-5},
        {"params": model.spatial_capsules.parameters(), "lr": 1e-5},
    ], weight_decay=3e-4)
    ...
    for g in optimizer.param_groups:
        g['lr'] *= 0.3  # reduce un 70%

    # =============================
    # CHG: quitar torch.compile
    # =============================
